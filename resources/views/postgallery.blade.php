...
<body style="background: lightgray;">
    <div class="container">
        <div>
            <h3 class="text">GALLERY</h3>
        </div>
        <div class="crbody">
            <a href="/homeadmin" class="btnh">HOME</a>
            <a href="{{ route('postsgalery.create') }}" class="btnsus">TAMBAH POST</a>
        </div>
        <table> 
            <thead>
                <tr>
              ...
              @forelse ($posts as $post)  
                <tr>
                    ...
                    <td class="text-center">
                        <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('postsgalery.destroy', $post->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
